...
import { ArrowLeft, FileText, Table2, Receipt, BookOpen, ListChecks, AlertTriangle, RefreshCw, ShieldAlert, Eye,  } from "lucide-react";
...
export default function CustomerWorkspace() {
  ...
  if (!session) {
    return (
      <div className="px-4 py-8">
        <p className="text-[0.875rem] leading-[1.2] text-[#db3743]">{error ?? "Customer not found."}</p>
        <button onClick={() => router.push("/customers")} className="text-[0.875rem] leading-[1.2] text-[#3c67ea] mt-2">← Back to customers</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 px-4 sm:px-6 py-5 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-[1rem] font-semibold leading-[1.2] text-[#242d48]">{session.company_name}</h1>
              <span className={cn("inline-flex items-center gap-1.5 text-[0.75rem] font-medium px-1 py-0 rounded-[2px] border uppercase", meta.pill)}>
      ...
      {/* Stepper */}
      <div className="rounded-[4px] bg-white border border-[#e6ebf8] shadow-[0_2px_4px_rgba(36,45,72,0.15)] px-4 py-3">
        <Stepper session={session} />
      </div>
      {/* Running banner */}
      {busy && (
        <div className="flex items-center gap-2 rounded-[4px] bg-[#e6ebf8] border border-[#e6ebf8] px-4 py-2.5 text-[13px] text-[#3c67ea]">
          <Loader size="inline" className="shrink-0" />
          {RUNNING_LABEL[session.status]}
        </div>
      )}
      {/* Failure banner */}
      {session.status === "failed" && (
        <div className="rounded-[4px] bg-white border border-[#f2919d] px-4 py-3 text-[13px] text-[#db3743]">
          <div className="flex items-center gap-2 font-medium">
            <AlertTriangle className="w-4 h-4 shrink-0" /> Pipeline failed
          </div>
          {session.errors.map((e, i) => (
            <p key={i} className="mt-1 text-[0.75rem]">
              <span className="font-mono font-semibold">{e.code}</span>{e.agent ? ` (${e.agent})` : ""}: {e.detail}
            </p>
          ))}
        </div>
      )}
      {error && <div className="text-[0.875rem] leading-[1.2] text-[#db3743] px-1">{error}</div>}
      {/* Tabs */}
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active ? "bg-white text-[#242d48] shadow-[0_2px_4px_rgba(36,45,72,0.15)]" : "text-[#485478] hover:text-[#242d48]"
              )}
            >
              <Icon className={cn("w-3.5 h-3.5", active ? "text-[#3c67ea]" : "text-[#485478]")} />
              {t.label}
            </button>
          );
        })}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[4px] bg-white border border-[#e6ebf8] shadow-[0_2px_4px_rgba(36,45,72,0.15)] p-4">
        <p className="text-[0.875rem] leading-[1.2] font-semibold text-[#242d48] mb-2">Uploaded documents</p>
          ...
          {session.uploaded_files.map((f) => {
            ...
            return (
              <li
                key={f.doc_id}
                className={cn(
                  "flex items-center gap-2 text-[0.875rem] leading-[1.2] rounded-[4px] -mx-2 px-2 py-1 transition-colors",
                  previewable && "cursor-pointer hover:bg-[#f0f1f7] group/file"
                )}
              ...
              >
                ...
                <span className={cn("truncate flex-1", previewable && "group-hover/file:text-[#3c67ea]")}>
                  {f.filename}
                </span>
                {previewable && (
                  <Eye className="w-3.5 h-3.5 text-[#485478] group-hover/file:text-[#3c67ea] transition-colors shrink-0" />
                )}
                {f.parse_status === "RUNNING" && (
                  <span className="inline-flex items-center gap-1 text-[0.75rem] text-[#3c67ea]"><Loader size="inline" />parsing</span>
                )}
                {f.parser && f.parse_status !== "RUNNING" && (
                  <span className="text-[0.75rem] text-[#485478]">{f.parser}</span>
                )}
                <span className="text-[0.75rem] font-medium px-1.5 py-0.5 rounded-[2px] bg-[#e6ebf8] text-[#3c67ea]">{f.doc_type}</span>
                <span className="text-[0.75rem] text-[#485478]">{f.page_count}p · {f.language}</span>
              </li>
            );
          })}
        </ul>
...
function AuditTab({ session }: { session: Session }) {
  ...
  return (
    <div className="space-y-4">
      {/* Per-agent timing */}
      <div className="rounded-[4px] bg-white border border-[#e6ebf8] shadow-[0_2px_4px_rgba(36,45,72,0.15)] p-4">
        <p className="text-[0.875rem] leading-[1.2] font-semibold text-[#242d48] mb-3">Agents</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {agents.map(({ key, out }) => (
            <div key={key} className="rounded-[4px] border border-[#e6ebf8] p-3">
              <p className="text-[0.75rem] uppercase tracking-[0.08em] text-[#485478]">{key}</p>
              <p className={cn(
                "text-[0.875rem] leading-[1.2] font-medium mt-0.5",
                out.status === "complete" && "text-[#14a687]",
                out.status === "failed" && "text-[#db3743]",
                out.status === "running" && "text-[#3c67ea]",
                out.status === "pending" && "text-[#485478]"
              )}>
                {out.status}
              </p>
              {out.duration_ms != null && (
                <p className="text-[0.75rem] text-[#485478] mt-0.5">{(out.duration_ms / 1000).toFixed(1)}s</p>
              )}
              {out.error_code && <p className="text-[0.75rem] text-[#db3743] mt-0.5 font-mono">{out.error_code}</p>}
            </div>
          ))}
        </div>
      </div>
      {/* Audit log */}
      <div className="rounded-[4px] bg-white border border-[#e6ebf8] shadow-[0_2px_4px_rgba(36,45,72,0.15)] p-4">
        <p className="text-[0.875rem] leading-[1.2] font-semibold text-[#242d48] mb-3">Audit log</p>
        <ul className="space-y-1.5">
          {session.audit_log.slice().reverse().map((a, i) => (
            <li key={i} className="flex items-start gap-3 text-[0.75rem]">
              <span className="text-[#485478] font-mono whitespace-nowrap shrink-0">{formatDate(a.ts)}</span>
              <span className="font-medium text-[#242d48]">{a.event.replace(/_/g, " ")}</span>
              {a.agent && <span className="text-[0.75rem] px-1.5 py-0.5 rounded-[2px] bg-[#e6ebf8] text-[#3c67ea]">{a.agent}</span>}
              {a.detail && <span className="text-[#485478] truncate">{a.detail}</span>}
            </li>
          ))}
        </ul>
      </div>
...
function Placeholder({ icon: Icon, text }: { icon: typeof FileText; text: string }) {
  return (
    <div className="rounded-[4px] bg-white border border-[#e6ebf8] shadow-[0_2px_4px_rgba(36,45,72,0.15)] p-10 text-center">
      <Icon className="w-6 h-6 text-[#3c67ea] mx-auto mb-2" />
      <p className="text-[0.875rem] leading-[1.2] text-[#485478]">{text}</p>
    </div>
  );
}

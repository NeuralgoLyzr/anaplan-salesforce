...
/**
 * SECURITY: LYZR_API_KEY is only used server-side here.
 * The browser never sees the key — it only calls /api/agent-traces.
 *
 * GET  → Read from MongoDB cache (fast, no Lyzr call)
 * POST → Sync from Lyzr v3/traces → upsert MongoDB → return fresh data
 */
...
function normalizeTrace(t: any): Trace {
  const durationMs = t.trace_duration != null
    ...
    : (t.total_duration_ms ?? t.duration_ms ?? 0);
  ...
  return {
    ...
    total_tokens: ((t.llm_input_tokens ?? 0) + (t.llm_output_tokens ?? 0)) || (t.total_tokens ?? 0),
...
async function syncFromLyzr(
  ...
  const url = new URL(`${LYZR_API_BASE}/v3/traces`);
  ...
  // API key is set in the server-side header — never sent to browser
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const rawList: any[] = raw.data ?? raw.traces ?? raw.items ?? (Array.isArray(raw) ? raw : []);

  const normalized = rawList.map(normalizeTrace).filter((t) => t.id);
  if (agentId) for (const t of normalized) t.agent_id = agentId;
  if (normalized.length) await upsertTraces(normalized);
  return normalized.length;
}

// GET — read MongoDB only (no Lyzr call, no API key needed)
export async function GET(req: NextRequest) {
  const { startTime, endTime, agentId, page, size } = parseParams(req);
  try {
    const { data, total } = await queryTraces({ agent_id: agentId, start_time: startTime, end_time: endTime, page, size });
...
// POST — sync from Lyzr (API key used server-side only) then return fresh data
export async function POST(req: NextRequest) {
  ...
  const { startTime, endTime, agentId, page, size } = parseParams(req);
  try { await syncFromLyzr(apiKey, agentId, startTime, endTime); } catch { /* fall through */ }

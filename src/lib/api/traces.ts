/**
 * Trace API Service
 * TypeScript interfaces and fetch logic for the Lyzr Trace endpoints.
 * All calls go through Next.js proxy routes (/api/traces/*) so the API key
 * is never exposed on the client.
 */
...
function getCacheKey(endpoint: string, params: Record<string, any>) {
  return `lyzr_cache_${endpoint}_${JSON.stringify(params)}`;
}
...
function normalizeTrace(t: any): Trace {
  ...
  return {
    ...
    total_tokens:
      ...
      (t.total_tokens ?? t.totalTokens ?? 0),
    cost,
    user_id: t.user_id ?? t.userId,
  };
}

export async function getTraceDashboard(
  filters?: TraceFilters
): Promise<DashboardMetrics> {
  const cacheKey = getCacheKey("dashboard", { ...filters });
  const cached = getCachedData<DashboardMetrics>(cacheKey);
  if (cached) return cached;

  const url = new URL("/api/traces/dashboard", window.location.origin);
  if (filters?.start_time) url.searchParams.append("start_time", filters.start_time);
  if (filters?.end_time) url.searchParams.append("end_time", filters.end_time);
  if (filters?.agent_id) url.searchParams.append("agent_id", filters.agent_id);
  ...
  const totalTok =
    raw.total_tokens ??
    (raw.total_input_tokens ?? 0) + (raw.total_output_tokens ?? 0);
  ...
  const daily: DailyMetric[] = (raw.daily_metrics ?? []).map((d: any) => ({
    ...
    total_tokens:
      d.total_tokens ??
      (d.total_input_tokens ?? 0) + (d.total_output_tokens ?? 0),
    input_tokens: d.total_input_tokens ?? d.input_tokens ?? undefined,
    output_tokens: d.total_output_tokens ?? d.output_tokens ?? undefined,
    avg_latency_ms: d.avg_latency_ms ?? 0,
    p95_latency_ms: d.p95_latency_ms ?? d.p95_latency ?? undefined,
...
// GET — reads from MongoDB only (fast, no Lyzr call)
...
// POST — syncs from Lyzr → upserts MongoDB → returns fresh data (Sync button)
...
export async function getTraceGantt(traceId: string): Promise<GanttChartData> {
  const cacheKey = getCacheKey(`gantt_${traceId}`, {});
  const cached = getCachedData<GanttChartData>(cacheKey);
  if (cached) return cached;
  ...
  function extractText(val: any): string | undefined {
    ...
    return val.content ?? val.text ?? val.message ?? JSON.stringify(val);
  }
  ...
  function spanType(name: string): GanttSpan["type"] {
    ...
    if (
      ...
      return "llm";
    if (n.includes("tool") || n === "memory" || n === "srs") return "tool";
  ...
  function flattenTree(node: any, parentId: string | null = null): any[] {
    ...
    for (const child of node.children ?? []) {
      flat.push(...flattenTree(child, node.span_id ?? node.id ?? ""));
    }
    return flat;
  }
  ...
  function buildSpan(s: any): GanttSpan {
    ...
    const endISO = startMs
      ? new Date(startMs + durMs).toISOString()
      : (s.end_time ?? startISO);
    const name = s.span_name ?? s.name ?? s.operation ?? "Span";
    const hasErr = s.hasError ?? s.has_error ?? false;
    const tagStatus = s.tags?.status ?? s.status ?? s.state ?? "";

    const inTok =
      s.tags?.llm_input_tokens ??
      s.tags?.input_tokens ??
      s.tags?.prompt_tokens ??
      0;
    const outTok =
      s.tags?.llm_output_tokens ??
      s.tags?.output_tokens ??
      s.tags?.completion_tokens ??
      0;
    const tokenTotal =
      (inTok + outTok) || s.tags?.tokens || s.tags?.token_count || 0;
    ...
    return {
      ...
      start_time: startISO,
      end_time: endISO,
      parent_id: s._parentId ?? s.parent_id ?? s.parentId ?? null,
      status: hasErr
        ? "error"
        : tagStatus === "error"
        ? "error"
  ...
  const normalized: GanttChartData = {
...
export async function getTraceSummary(traceId: string): Promise<TraceSummary> {
  const cacheKey = getCacheKey(`summary_${traceId}`, {});
  const cached = getCachedData<TraceSummary>(cacheKey);
  if (cached) return cached;
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function toText(val: any): string | undefined {
    if (!val) return undefined;
    if (typeof val === "string") return val;
    return val.content ?? val.text ?? val.message ?? JSON.stringify(val);
  }
  ...
  const normalized: TraceSummary = {
    ...
    duration_ms:
      ...
      raw.duration_ms ??
      raw.durationMs ??
      (raw.duration_seconds ? raw.duration_seconds * 1000 : 0),
    input_tokens: inTok,
    output_tokens: outTok,
    total_tokens: raw.total_tokens ?? raw.totalTokens ?? inTok + outTok,
    credits: (raw.total_cost ?? raw.credits ?? raw.cost ?? raw.credit ?? 0) / 100,
...
// Re-export normalizeTrace for use in API routes
export { normalizeTrace };
